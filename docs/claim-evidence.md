...
Implementation status distinguishes local execution, verified GitHub Actions evidence, public repository evidence, and real-provider or deployment evidence. Public source availability does not convert planned Stage 2 or measurement claims into implemented evidence. A workflow file alone is configuration; runs `32924165501`, `33012384259`, and the current release-head Actions run are actual remote CI evidence.

| Claim target | Current status | Exact evidence | Missing evidence | Stage |
|---|---|---|---|---|
| Low-level LangGraph orchestration | Implemented, remotely verified, and publicly inspectable | Public repository; `build_graph()` uses `StateGraph`, `START`, `END`; relevant source and tests; local Python 3.13.15 verification; Actions execution on Python 3.11.16, 3.12.14, and 3.13.15 | General task coverage, real-provider behavior, and deployment evidence | Stage 1 |
...
| CI configuration and test evidence | Implemented, remotely verified, and public | `.github/workflows/ci.yml`; runs `32924165501` and `33012384259`; the current release-head Actions run; Python 3.11.16, 3.12.14, and 3.13.15; 277 tests passed in each job; Ruff, format, strict mypy, schema synchronization, deterministic generation, generated/checked artifact validation, and semantic verification passed in each job | Another operating system, deployment, and real-provider behavior | Stage 1 |
...
On 2026-08-25, authoritative Python 3.13.15 local verification reported Ruff clean, Ruff format clean, strict mypy clean, all 277 deterministic tests passing, synchronized artifact schema 0.3.0, successful deterministic CLI generation, valid generated and checked artifacts, exact causal reconciliation, and equal semantic reproduction. Private-stage GitHub Actions runs `32924165501` and `33012384259` independently passed those checks in each Python 3.11.16, 3.12.14, and 3.13.15 matrix job. Public repository availability followed the successful final release-head CI gate. This is local, remote-CI, and public-source evidence, but it remains fixture-only and is not deployment or real-provider evidence. Hashes are modification detectors, not signatures.
