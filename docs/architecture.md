...
Artifact validation separately proves the exact successful four-turn contract, request/attempt identity, canonical request digests, terminal response equality to accepted plan/calls/decision, exact attempt-event cardinality and context, coherent failure references, accepted worker/tool reconciliation, lifecycle order, UTC timestamps, and the same pure semantics used live. Successful artifacts reject missing and unrelated turns. Failed/cancelled artifacts allow only legitimately reached turns and cannot contain downstream state without the required terminal response.
