...
Version `0.3.0` artifacts add durable internal structured model requests and stable failure IDs to task/config/state/decision, event, provider-response, tool, accounting, provenance, and integrity evidence. Artifact versions `0.1.0` and `0.2.0` remain unchanged historical schemas; unchanged task, config, model-fixture, order-fixture, and policy-fixture shapes remain version `0.2.0`. Each request is recorded once before its first attempt, retries reference it, and its payload SHA-256 covers canonical JSON of the full persisted payload. Successful runs contain exactly the planner, order worker, policy worker, and finalizer turns. Terminal responses must parse through the same strict public models and equal the persisted plan, call, or decision.

Model/tool attempt events reconcile exactly to record identity, source, phase, parent span, attempt number, outcome, and failure reference. Stable `failure_id` values connect nested attempt/worker failures, top-level failures, and failure/terminal events. A failed worker cannot contradict terminal successful tool evidence. Cancellation and unexpected-failure assembly use a run-scoped accepted-state snapshot. `LoadedInputs` constructs a fresh fixture provider for every execution. Request persistence is not chain-of-thought capture; for the fixture adapter the request is the complete provider input, while a later real adapter may require separate redacted provider-wire provenance. Hashes remain modification detectors, not signatures.
